...
DWORD64 __cdecl GetModuleHandle64(wchar_t* lpModuleName)
{
    ...
    _nPEB64 peb64;
    getMem64(&peb64, teb64.ProcessEnvironmentBlock, sizeof(_nPEB64));
    _nPEB_LDR_DATA64 ldr;
    getMem64(&ldr, peb64.Ldr, sizeof(_nPEB_LDR_DATA64));

    DWORD64 LastEntry = peb64.Ldr + offsetof(_nPEB_LDR_DATA64, InLoadOrderModuleList);
    _nLDR_DATA_TABLE_ENTRY64 head;
    head.InLoadOrderLinks.Flink = ldr.InLoadOrderModuleList.Flink;
    do
    {
        getMem64(&head, head.InLoadOrderLinks.Flink, sizeof(_nLDR_DATA_TABLE_ENTRY64));
...
SIZE_T __cdecl VirtualQueryEx64(HANDLE hProcess, DWORD64 lpAddress, MEMORY_BASIC_INFORMATION64* lpBuffer, SIZE_T dwLength)
{
    ...
    if (0 == ntqvm)
    {
        ntqvm = GetProcAddress64(getNTDLL64(), (char*)"NtQueryVirtualMemory");
        if (0 == ntqvm)
            return 0;
    }
    DWORD64 ret = 0;
...
DWORD64 __cdecl VirtualAllocEx64(HANDLE hProcess, DWORD64 lpAddress, SIZE_T dwSize, DWORD flAllocationType, DWORD flProtect)
{
    ...
    if (0 == ntavm)
    {
        ntavm = GetProcAddress64(getNTDLL64(), (char*)"NtAllocateVirtualMemory");
        if (0 == ntavm)
            return 0;
    }
...
BOOL __cdecl VirtualFreeEx64(HANDLE hProcess, DWORD64 lpAddress, SIZE_T dwSize, DWORD dwFreeType)
{
    ...
    if (0 == ntfvm)
    {
        ntfvm = GetProcAddress64(getNTDLL64(), (char*)"NtFreeVirtualMemory");
        if (0 == ntfvm)
            return 0;
    }
...
BOOL __cdecl VirtualProtectEx64(HANDLE hProcess, DWORD64 lpAddress, SIZE_T dwSize, DWORD flNewProtect, DWORD* lpflOldProtect)
{
    ...
    if (0 == ntpvm)
    {
        ntpvm = GetProcAddress64(getNTDLL64(), (char*)"NtProtectVirtualMemory");
        if (0 == ntpvm)
            return 0;
    }
...
BOOL __cdecl ReadProcessMemory64(HANDLE hProcess, DWORD64 lpBaseAddress, LPVOID lpBuffer, SIZE_T nSize, SIZE_T *lpNumberOfBytesRead)
{
    ...
    if (0 == nrvm)
    {
        nrvm = GetProcAddress64(getNTDLL64(), (char*)"NtReadVirtualMemory");
        if (0 == nrvm)
            return 0;
    }
...
BOOL __cdecl WriteProcessMemory64(HANDLE hProcess, DWORD64 lpBaseAddress, LPVOID lpBuffer, SIZE_T nSize, SIZE_T *lpNumberOfBytesWritten)
{
    ...
    if (0 == nrvm)
    {
        nrvm = GetProcAddress64(getNTDLL64(),  (char*)"NtWriteVirtualMemory");
        if (0 == nrvm)
            return 0;
    }
...
BOOL __cdecl GetThreadContext64(HANDLE hThread, _CONTEXT64* lpContext)
{
    ...
    if (0 == gtc)
    {
        gtc = GetProcAddress64(getNTDLL64(), (char*)"NtGetContextThread");
        if (0 == gtc)
            return 0;
    }
...
BOOL __cdecl SetThreadContext64(HANDLE hThread, _CONTEXT64* lpContext)
{
    ...
    if (0 == stc)
    {
        stc = GetProcAddress64(getNTDLL64(), (char*)"NtSetContextThread");
        if (0 == stc)
            return 0;
    }
