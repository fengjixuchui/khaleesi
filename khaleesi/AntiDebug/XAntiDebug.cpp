...
XAD_STATUS XAntiDebug::XAD_Initialize()
{
	...
	if (_flags & FLAG_DETECT_DEBUGGER)
	{
		if (_isArch64)
		{
			...
			InitWow64Ext();
#endif
			_MyQueryInfomationProcess = (DWORD64)GetProcAddress64(GetModuleHandle64((wchar_t*)XAD_NTDLL), (char*)"ZwQueryInformationProcess");
			if (_MyQueryInfomationProcess == NULL)
			{
				return XAD_ERROR_NTAPI;
			}
			_MyQueryInfomationProcess -= (DWORD64)GetModuleHandle64((wchar_t*)XAD_NTDLL);
		}
		else
		{
#ifndef _WIN64
		...
		if (_isArch64)
		{
			...
			getMem64(pehead, GetModuleHandle64((wchar_t*)XAD_NTDLL), XAD_PAGESIZE);
