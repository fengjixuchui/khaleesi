...
void PageExceptionInitialEnum()
{
	...
	if (hash_GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, (LPCWSTR)PageExceptionBreakpointCheck, &hMainModule))
	{
		...
		if (GetModuleInformation(hash_GetCurrentProcess(), hMainModule, &moduleInfo, sizeof(MODULEINFO)))
		{
			...
			{
				if (hash_VirtualQuery(module + ofs, &memInfo, sizeof(MEMORY_BASIC_INFORMATION)) >= sizeof(MEMORY_BASIC_INFORMATION))
				{
...
BOOL PageExceptionBreakpointCheck()
{
	...
	if (hash_GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, (LPCWSTR)PageExceptionBreakpointCheck, &hMainModule))
	{
		...
		for (PVOID page : executablePages)
		{
			...
			if (hash_VirtualQuery(page, &memInfo, sizeof(MEMORY_BASIC_INFORMATION)) >= sizeof(MEMORY_BASIC_INFORMATION))
			{
				if (memInfo.AllocationProtect == 0)
					OutputDebugStringDbgOnly(L"^ AllocationProtect is zero. Potential shenanigans.");
				if (memInfo.Protect == 0)
